...
async def scout_preview(query:str,limit:int=6)->dict:
    ...
    try:
        ...
        for item in items[:limit]:
            rows.append({"provisional":True,"candidate":{"login":item.get("login"),"name":item.get("name"),"profile_url":item.get("profile_url"),"followers":item.get("followers",0),"primary_repository":item.get("primary_repository")},"activity":{"events_7d":item.get("recent_events_7d",0),"events_30d":item.get("recent_events_30d",0)},"status":"VALIDATING"})
...
async def scout(query:str,limit:int=20)->dict:
    ...
    try:
        ...
        for item in items:
            ...
            if quality.get("available") and top.get("url"):evidence.append(Evidence(type="merged_pull_request",source="github",source_url=top["url"],subject_id=sid,observed_at=top.get("merged_or_closed_at"),value={"repository":quality.get("repository"),"number":top.get("number"),"title":top.get("title"),"core_file_count":top.get("core_file_count",0),"core_files":top.get("core_files",[]),"keyword_hits":top.get("keyword_hits",[]),"additions":top.get("additions",0),"deletions":top.get("deletions",0),"substantive":top.get("substantive",False)}))
