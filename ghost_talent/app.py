...
async def run_scout_preview(q: str = Query(min_length=2, max_length=120), limit: int = 6):
    try:
        ...
        raise HTTPException(status_code=502, detail={"code": "preview_failed", "message": str(exc)}) from exc
...
async def run_scout(q: str = Query(min_length=2, max_length=120), limit: int = 20):
    try:
        ...
        if not allowed:
            error = scout_result.get("error") or {
                "code": "discovery_incomplete",
                "message": "Discovery evidence is incomplete. No snapshot was written.",
            }
            raise HTTPException(
                status_code=503,
                detail={
                    **error,
                    ...
                    "integrity_gate": reason,
                    "sources": scout_result.get("sources", {}),
                },
            )
        ...
        for row in results:
            ...
            row["trajectory"] = trajectories.get(
                login,
                {
                    "observation_count": 1,
        ...
        return {
            ...
            "results": results,
            "sources": sources,
            "source_health": scout_result.get("source_health", {}),
            "snapshot": snapshot,
...
async def delete_watchlist(login: str):
    ...
    filtered = [
        item
        for item in items
        if str((item.get("candidate") or {}).get("login") or "").lower() != login.lower()
    ]
...
async def validation():
    ...
    public = sum(1 for row in cohorts if row.get("artifact_status") == "public-frozen")
    committed = sum(1 for row in cohorts if row.get("freeze_time_commitment"))
    return {
        "protocol": "v2",
        ...
        "summary": {
            ...
            "public_frozen": public,
            "freeze_time_committed": committed,
