...
def _external(e):
    substantive=int(e.get("substantive_external_prs",e.get("external_merged_prs",0)));recognized=int(e.get("recognized_upstream_prs",0));core=int(e.get("core_path_prs",0));accepted=int(e.get("maintainer_accepted_prs",0))
    return _clamp(min(30,11*math.log1p(substantive))+min(30,19*math.log1p(recognized))+min(20,13*math.log1p(core))+min(20,14*math.log1p(accepted)))
...
def score_candidate(c:Candidate)->dict:
    ...
    external_count=int(ext.get("external_merged_prs",0)) if external_available else 0;substantive_count=int(ext.get("substantive_external_prs",external_count)) if external_available else 0;recognized_count=int(ext.get("recognized_upstream_prs",0)) if external_available else 0;core_count=int(ext.get("core_path_prs",0)) if external_available else 0;accepted_count=int(ext.get("maintainer_accepted_prs",0)) if external_available else 0;accepted_core_count=int(ext.get("maintainer_accepted_core_path_prs",0)) if external_available else 0;recent_external=int(ext.get("recent_external_merged_prs",0)) if external_available else 0;recent_recognized=int(ext.get("recent_recognized_upstream_prs",0)) if external_available else 0;recent_core=int(ext.get("recent_core_path_prs",0)) if external_available else 0;recent_accepted=int(ext.get("recent_maintainer_accepted_prs",0)) if external_available else 0;recent_accepted_core=int(ext.get("recent_maintainer_accepted_core_path_prs",0)) if external_available else 0
    ...
    external_pr_units=substantive_count*20+recognized_count*25+core_count*15+accepted_count*20;research_units=sum(1 for p in c.paper_matches if p.get("identity_status")=="verified_external_link")*20;total=max(self_repo_evidence+external_repo_evidence+unclassified_repo_evidence+external_pr_units+research_units,1);mix={"self_owned_repo_pct":round(100*self_repo_evidence/total,1),"external_repository_contribution_pct":round(100*external_repo_evidence/total,1),"verified_external_pr_pct":round(100*external_pr_units/total,1),"verified_research_pct":round(100*research_units/total,1),"unclassified_repository_pct":round(100*unclassified_repo_evidence/total,1)}
    confidence=35+min(12,4*len(c.repositories))+(8 if c.name else 0)+(10 if substantive_count else 0)+(8 if recognized_count else 0)+(8 if accepted_count else 0);noise_clear=(c.noise or {}).get("status","clear")=="clear";concentration=mix["self_owned_repo_pct"]>80 and substantive_count==0
    ...
    already_visible=bool(external_available and external>=60 and c.followers>=500);emerging_visibility=bool(c.followers<500);accepted_core=bool(recent_accepted_core>=1);credible_upstream=bool(recent_recognized>=1 and (recent_accepted>=1 or recent_core>=1));recent_external_signal=bool(recent_external>=1)
    ...
    elif radar>=65 and external>=50 and confidence>=55 and emerging_visibility and credible_upstream and history_sufficient:status="EARLY SIGNAL"
    ...
    if recognized_count:reasons.append(f"{recognized_count} substantive recognized-upstream PR(s)")
    if core_count:reasons.append(f"{core_count} changed-file verified core-path PR(s)")
    if accepted_count:reasons.append(f"{accepted_count} owner/member-approved substantive upstream PR(s)")
