...
def score_candidate(c:Candidate)->dict:
 ...
 external_count=int(ext.get("external_merged_prs",0)) if external_available else 0;recognized_count=int(ext.get("recognized_upstream_prs",0)) if external_available else 0;core_count=int(ext.get("core_path_prs",0)) if external_available else 0;accepted_count=int(ext.get("maintainer_accepted_prs",0)) if external_available else 0;self_evidence=max(sum(int(r.get("contributions",0)) for r in c.repositories),0);external_units=external_count*20+recognized_count*25+core_count*15+accepted_count*20;research_units=sum(1 for p in c.paper_matches if p.get("identity_status")=="verified_external_link")*20;total=max(self_evidence+external_units+research_units,1);mix={"self_owned_or_discovery_repo_pct":round(100*self_evidence/total,1),"external_upstream_pct":round(100*external_units/total,1),"verified_research_pct":round(100*research_units/total,1)}
 confidence=35+min(12,4*len(c.repositories))+(8 if c.name else 0)+(10 if external_count else 0)+(8 if recognized_count else 0)+(8 if accepted_count else 0);noise_clear=(c.noise or {}).get("status","clear")=="clear";concentration=mix["self_owned_or_discovery_repo_pct"]>80 and external_count==0
 ...
 already_visible=bool(external_available and external>=60 and c.followers>=500);emerging_visibility=bool(c.followers<500);accepted_core=bool(accepted_count>=1 and core_count>=1);credible_upstream=bool(recognized_count>=1 and (accepted_count>=1 or core_count>=1))
