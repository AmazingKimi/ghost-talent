...
class GitHubSource:
    ...
    def _candidate(candidates: dict[str, dict], login: str, profile_url: str | None = None) -> dict:
        item = candidates[login]; item["login"] = login; item["profile_url"] = profile_url or item.get("profile_url") or f"https://github.com/{login}"
        item.setdefault("repositories", []); item.setdefault("discovery_sources", []); item.setdefault("merged_pr_discoveries", []); return item
    ...
    async def discover(self,query:str,repo_limit:int=15,candidate_limit:int=20,contributor_limit:int=25,quality_budget:int=12,pr_repo_budget:int=10,pr_limit:int=20)->list[dict]:
        ...
        for repo,contributors in await asyncio.gather(*(load_contributors(repo) for repo in repositories)):
            for contributor in contributors:
                ...
                item=self._candidate(candidates,contributor["login"],contributor.get("html_url"))
        ...
        if self.authenticated:
            for repo,pulls in await asyncio.gather(*(load_pulls(repo) for repo in repositories[:pr_repo_budget])):
                for pull in pulls:
                    ...
                    if not login or user.get("type")=="Bot" or login.endswith("[bot]"):continue
        ...
        raw_ranked=sorted(candidates.values(),key=lambda c:(len(c.get("merged_pr_discoveries",[]))>0,sum(r["contributions"] for r in c.get("repositories",[])),len(c.get("repositories",[])),sum(r["stars"] for r in c.get("repositories",[]))),reverse=True)
        ranked=self._diversify(raw_ranked,candidate_limit,max(2,candidate_limit//5))
        async def enrich(index,item):
            ...
            return {**item,"primary_repository":self._primary_repo(item),"name":profile.get("name") if profile else None,"profile_url":profile.get("html_url") if profile else item["profile_url"],"followers":profile.get("followers",0) if profile else 0,"github_enrichment_complete":profile is not None,"contribution_quality":quality,**counts}
    ...
    async def _contribution_quality(self,item:dict)->dict:
        ...
        try:files=await self._get(f"{API}/repos/{repo_name}/pulls/{top_pr['number']}/files",per_page=30)
            ...
            if exc.response.status_code not in {403,404,429}:raise
        filenames=[str(file.get("filename","")) for file in files];core_files=[name for name in filenames if self._is_core_path(name)];title=str(top_pr.get("title") or "")
        keyword_hits=sorted({term for term in CORE_TERMS if term in title.lower() or any(term in name.lower() for name in filenames)})
        patch_text="\n".join(str(file.get("patch") or "") for file in files if file.get("patch"))[:12000]
        patch_lower=patch_text.lower();patch_keyword_hits=sorted({term for term in CORE_TERMS if term in patch_lower})
        code_lines=[line for line in patch_text.splitlines() if line.startswith(("+","-")) and not line.startswith(("+++","---"))]
